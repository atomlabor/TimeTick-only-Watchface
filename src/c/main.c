...
#define MSG_KEY_ENABLE_WEATHER  5
#define MSG_KEY_ENABLE_CLEANSTYLE 7

#define PERSIST_KEY_WEATHER_ENABLED 100
#define PERSIST_KEY_CLEANSTYLE      101
...
static Layer *s_main_cleanstyle_layer, *s_timer_cleanstyle_layer;
...
static bool s_cleanstyle_enabled = false;
...
static void cleanstyle_update_proc(Layer *layer, GContext *ctx);
...
static void cleanstyle_update_proc(Layer *layer, GContext *ctx) {
  if (!s_cleanstyle_enabled) return;

  GRect bounds = layer_get_bounds(layer);

  graphics_context_set_fill_color(ctx, GColorBlack);
  graphics_fill_rect(ctx, GRect(0, 0, bounds.size.w, 16), 0, GCornerNone);
  graphics_fill_rect(ctx, GRect(0, bounds.size.h - 16, bounds.size.w, 16), 0, GCornerNone);
}
...
static void hal_play_alarm(bool en) {
  if (en) {
    ...
    vibes_enqueue_custom_pattern((VibePattern) {
      .durations = (uint32_t[]) {400, 200, 1000},
      .num_segments = 3
    });
  } else {
    vibes_cancel();
...
static void inbox_received_handler(DictionaryIterator *iter, void *ctx) {
  ...
  Tuple *cleanstyle_t = dict_find(iter, MSG_KEY_ENABLE_CLEANSTYLE);
  if (cleanstyle_t) {
    s_cleanstyle_enabled = (cleanstyle_t->value->int32 == 1);
    persist_write_bool(PERSIST_KEY_CLEANSTYLE, s_cleanstyle_enabled);

    if (s_main_cleanstyle_layer) {
      layer_mark_dirty(s_main_cleanstyle_layer);
    }

    if (s_timer_cleanstyle_layer) {
      layer_mark_dirty(s_timer_cleanstyle_layer);
    }
  }

  Tuple *weather_enabled_t = dict_find(iter, MSG_KEY_ENABLE_WEATHER);
  if (weather_enabled_t) {
    s_weather_enabled = (weather_enabled_t->value->int32 == 1);
    persist_write_bool(PERSIST_KEY_WEATHER_ENABLED, s_weather_enabled);

    if (!s_weather_enabled && s_weather_layer) {
      layer_set_hidden(text_layer_get_layer(s_weather_layer), true);
    }
  }

  Tuple *t = dict_find(iter, MSG_KEY_TEMP);
  if (t && s_weather_enabled) {
    int32_t temp = 0;

    if (t->type == TUPLE_CSTRING) {
      temp = (int32_t)atoi(t->value->cstring);
    } else {
      switch (t->length) {
        case 1:
          temp = (int32_t)(t->type == TUPLE_INT ? t->value->int8 : (int8_t)t->value->uint8);
          break;

        case 2:
          temp = (int32_t)(t->type == TUPLE_INT ? t->value->int16 : (int16_t)t->value->uint16);
          break;

        case 4:
          temp = (int32_t)(t->type == TUPLE_INT ? t->value->int32 : (int32_t)t->value->uint32);
          break;

        default:
          temp = (int32_t)t->value->int32;
          break;
      }
    }
    ...
    if (temp == 99) {
      ...
      APP_LOG(APP_LOG_LEVEL_INFO, "No weather API key. Weather hidden.");
      return;
    }

    if (temp == 95) {
      snprintf(s_weather_buf, sizeof(s_weather_buf), "NO GPS");
    } else if (temp == 94) {
      snprintf(s_weather_buf, sizeof(s_weather_buf), "NO NET");
    } else if (temp == 93 || temp == 92) {
      snprintf(s_weather_buf, sizeof(s_weather_buf), "API ERR");
    } else {
      snprintf(s_weather_buf, sizeof(s_weather_buf), "%ld°C", (long)temp);
    }
    ...
    APP_LOG(APP_LOG_LEVEL_INFO, "Temperature received: %s", s_weather_buf);
  }
}
...
static void request_weather(void) {
  if (!s_weather_enabled || !s_weather_layer) return;
  ...
  if (begin_result != APP_MSG_OK || iter == NULL) {
    APP_LOG(APP_LOG_LEVEL_ERROR, "Weather request begin failed: %d", begin_result);
    layer_set_hidden(text_layer_get_layer(s_weather_layer), true);
    return;
  }
...
static void main_window_load(Window *window) {
  ...
  s_main_cleanstyle_layer = layer_create(GRect(0, 0, 200, 228));
  layer_set_update_proc(s_main_cleanstyle_layer, cleanstyle_update_proc);
  layer_add_child(root, s_main_cleanstyle_layer);
...
static void main_window_unload(Window *window) {
  ...
  if (s_main_cleanstyle_layer) {
    layer_destroy(s_main_cleanstyle_layer);
    s_main_cleanstyle_layer = NULL;
  }
...
static void timer_window_load(Window *window) {
  ...
  s_timer_cleanstyle_layer = layer_create(GRect(0, 0, 200, 228));
  layer_set_update_proc(s_timer_cleanstyle_layer, cleanstyle_update_proc);
  layer_add_child(root, s_timer_cleanstyle_layer);
...
static void timer_window_unload(Window *window) {
  ...
  if (s_timer_cleanstyle_layer) {
    layer_destroy(s_timer_cleanstyle_layer);
    s_timer_cleanstyle_layer = NULL;
  }
...
static void accel_tap_handler(AccelAxisType axis, int32_t direction) {
  ...
  if (top == s_main_window && s_weather_enabled) {
    request_weather();
  }
}
...
static void init(void) {
  ...
  if (persist_exists(PERSIST_KEY_CLEANSTYLE)) {
    s_cleanstyle_enabled = persist_read_bool(PERSIST_KEY_CLEANSTYLE);
  }
